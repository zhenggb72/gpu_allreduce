...
public:
    ...
    void init(sycl::queue& queue, uint32_t rank_in, uint32_t world_in){
        ...
        queue.fill<int>((int*)((data_type *)local_buffer + world * max_buffer * 1024/ sizeof(data_type)), 0, 1024/sizeof(int)); 
        queue.wait();
...
private:
    void exchange_peer_ipc_mem(sycl::queue& queue, void* ptr) {
        ...
        for (uint32_t i = 0; i < world; i++){
            ...
            sync_buffer[i] = (char*)peer_base + peer->offset + world * max_buffer * 1024;
